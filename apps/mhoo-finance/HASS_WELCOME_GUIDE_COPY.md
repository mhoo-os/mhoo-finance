# Hass Welcome Guide — Finance drop-in copy

Status: copy proposal for the private Hass Welcome Guide. Do not publish from this repository. The guide owner must review and place it without changing the existing email gate, invite flow, Clover instructions or access-cleanup files.

## Short pitch

### Your Hass Workspace, starting with clearer finances

Hass Workspace is the private place for your team, records, connections and review work. Finance is the first focused experience inside it: bring permitted financial evidence together, see what each source supports, investigate differences and keep the path back to the original record.
...
Finance organizes permitted tax returns, bank or Plaid transactions, Clover sales and settlements, and invoice or vendor evidence by their real source period. Missing and partial periods stay visible. One source is never rewritten just to make another source agree.
...
The current view is a synthetic demonstration of the Finance experience. It shows the question-to-evidence journey, explicit gaps and simulated error/permission states. It does not connect to Hass accounts, read live email, prove complete history or produce a real financial conclusion.

## What comes next

Live source onboarding, complete-period evidence, reviewed reconciliation and forecasting require separate source authorization, accepted Workspace permissions, retention decisions and verification. The existing Twenty Workspace remains the owner of identity, roles, connections, encrypted credentials and records.

## Security and evidence wording

- Use only the Workspace's existing invitation and sign-in path.
- Paste provider credentials only inside the authorized Workspace connection form, never in a message.
...
**Finance — understand yesterday, review today, plan tomorrow**

Follow a clear path from a question to the chart, the contributing transactions and the available source evidence. Differences stay visible, uncertain matches wait for human review, and future scenarios stay tied to explicit assumptions.
